...
> **Validator này CÓ THẬT và đã chạy on-chain — không phải thiết kế.** Mã nguồn KHÔNG nằm ở repo Strata (đúng ranh giới "Mosaic giữ validator", `docs/STRATA-S1-REPORT.md §4`) mà ở **`VeDataIO/Core: mosaic/aiken/validators/strata_anchor.ak` + `lib/strata/anchor.ak`** (Plutus V3, blueprint `mosaic/aiken/plutus.json`). Đã chứng minh đầu-cuối trên Preview: happy `seq 1→2` và rollback `1→0` bị node từ chối phase-2 (`STRATA-S1-REPORT.md §4.1`). Tìm `.ak` trong repo Strata sẽ ra 0 file — đó là kỳ vọng, KHÔNG phải bằng chứng validator chưa tồn tại.
...
> ⚠️ **Lệch pha off-chain ↔ on-chain — anh Đức chốt hướng (B) ngày 2026-08-07, bản vá đã land.** Tầng đẩy anchor off-chain **cho gap**: `chain.rs` `publish_anchor` chỉ từ chối `seq <= prev`, `settlement.rs` `publish_batch` chỉ chặn `on_chain.seq > a.seq`. Validator trên chain **cấm gap** (`anchor.ak:55-57`, property test `prop_seq_advances_rejects_skip` cấm nhảy bậc tường minh). Hệ quả trên đường Mosaic-A: anchor `seq=1` fail → head local nhảy tới 2 → retry neo `seq=2` bị validator từ chối **vĩnh viễn** vì `2 ≠ 0+1` ⇒ **chuỗi anchor wedge**. Mâu thuẫn nằm giữa HAI repo (Strata core ↔ VeDataIO/Core), nên sửa mình spec không gỡ được. Hai hướng, chi phí thật:
...
> - **(B) Giữ `== seq + 1` on-chain**, sửa tầng đẩy cho riêng đường Mosaic-A: neo **đúng từng seq / retry đúng seq đang kẹt** thay vì neo head hiện hành. Chi phí: 1 tx mỗi version (không neo thưa được) + ràng liveness của append vào finality của chain. ← **hướng đã chọn**; lý do anh Đức nêu: dùng Mosaic cho nhất quán, vì sau này Mosaic còn đảm nhận cả việc trả phí L1.
>
> Bản vá land ở `#42`: `MosaicAnchorSink::publish` chặn **tại chỗ, trước khi dựng tx** bằng `AnchorError::SeqGap` (`src/anchor_sink.rs:76` và `:755`; hình dạng + phạm vi ở `API §8.1(b)`). Nên luật ghi ở đây (`== seq + 1`) nay là luật của **cả hai tầng**, không còn lệch pha: tầng đẩy Mosaic-A coi gap là điều kiện lỗi, không phải sparsity lành tính. Một ngoại lệ đúng phạm vi: lần neo **đầu tiên** của một lineage không bị chặn, vì validator chỉ guard SPEND. Ở tầng **Settlement** (metadata, không validator) gap vẫn lành tính vì `mmr_root` tại `seq` neo đã cam kết trọn `0..=seq`. Contiguity `+1` giữa các *version* là INV-E2, tầng khác, không liên quan.
>
> ⚠️ **Validator KHÔNG ràng `mmr_root'` với `mmr_root` — và `Strata-Math §7.1` nay nói đúng điều đó.** Validator đang chạy kiểm `mmr_root` **chỉ ở độ dài 32B**. Hệ quả thật: `seq` đơn điệu chặn **tụt lùi** độc-lập-khoá, nhưng KHÔNG ràng `mmr_root'` phải là **mở rộng** của `mmr_root`; kẻ chiếm khoá author dựng được một nhánh lịch sử KHÁC rồi neo tiến lên với `seq` cao hơn và validator vẫn nhận. Chống rewrite hiện dựa vào **khoá author + ngưỡng operator**, KHÔNG dựa vào cấu trúc MMR.
>
> *Lịch sử của chỗ này, giữ lại vì nó là một quyết định chứ không phải một lỗi chính tả.* Bản cũ của `Strata-Math §7.1` phát biểu hợp đồng validator gồm **ba** vế, vế thứ ba là *"inclusion-proof: lá của head cũ vẫn nằm dưới `mmr_root` mới"*. **Anh Đức chốt 2026-08-13: chuẩn là §5.4 này, `Strata-Math §7.1` mới là chỗ sai và phải sửa theo** — sai kể cả nếu vế thứ ba được hiện thực, vì inclusion-proof của **một** lá chỉ ràng buộc các nút trên đường xác thực của đúng lá đó, mọi lá khác vẫn tự do bị viết lại mà proof ấy vẫn verify. Thứ phát biểu được "mới là phần mở rộng của cũ" là **consistency-proof** (toàn bộ tập peak cũ đều là nút của MMR mới và fold lại đúng `mmr_root_cũ`). **Quyết định ấy nay đã thi hành:** `Strata-Math §7.1` viết lại thành bốn mục (a)–(d), Mệnh đề 1 và Mệnh đề 2 hạ phạm vi theo. Nên §5.4 này và Math §7.1 **không còn mâu thuẫn** — đừng đọc đoạn này thành "Math vẫn đang sai".
>
> Vế đúng vẫn **chưa dựng được**: crate MMR nền chỉ có inclusion (`git grep -c consistency -- src/` trên `origin/main@fbc643c` trả **0**), và validator không nhận proof nào trong redeemer. Ngôn ngữ "độc-lập-khoá" ở `API §8.1(b)` đã tách rõ *chống-tụt-lùi* với *chống-rewrite*.
>
> ✅ **Nợ này đã đóng.** Trước đây mục này ghi `Strata-Math §7.1` còn viết vế inclusion và `§10 Mệnh đề 2` còn phát biểu quá mức (nói vượt INV-E7 đòi va chạm băm hoặc giả mạo chữ ký, trong khi có đường thứ ba: người có khoá tác giả dựng một lịch sử **khác** tại `seq+1` rồi neo đè, chữ ký thật và băm thật). Cả hai nay đã sửa: §7.1 tách bốn mục (a)–(d), Mệnh đề 2 đổi tên thành *"chống rollback theo `seq`, phạm vi HẸP"* và mang khối giới hạn nói thẳng đường thứ ba, Mệnh đề 1 khai thêm giả thiết **khoá tác giả không bị lộ**.
>
