...
- **INV-E3** (append-only history): thêm version chỉ MỞ RỘNG mmr; đường lá→đỉnh của inclusion-proof cũ giữ nguyên, verify LẠI dưới tập peak hiện hành (KHÔNG phải byte-proof bất biến — chuẩn hình thức: `Strata-Math §4.5`).
...
- **INV-E7** (chống rollback): anchor on-chain đơn điệu theo `seq`; không thể neo lại version cũ. Phạm vi hẹp — chỉ *chống tụt lùi*, KHÔNG bao gồm *chống rewrite-then-re-anchor* (vế còn thiếu là **consistency-proof** giữa `mmr_root_cũ` và `mmr_root_mới` — nó chưa có ở đâu: không trong crate MMR, không trong redeemer validator. `Strata-Math §7.1(c)(d)`; mức đảm bảo theo từng backend: `Strata-API §8.1(b)`).
